...
@AndroidEntryPoint
class MemberTrackerConfigActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Default to CANCELED so backing out of the picker doesn't materialize
        // an unconfigured widget on the home screen.
                    ...
                    items(current, key = { it.id }) { m ->
                        MemberPickRow(
                            ...
                            onToggle = { wantChecked ->
                                selected = if (wantChecked) selected + m.id else selected - m.id
                            },
                        )
                    }
                }
...
private fun MemberPickRow(
    ...
    Row(
        ...
        Column(modifier = Modifier.weight(1f)) {
            ...
            member.pronouns?.takeIf { it.isNotBlank() }?.let {
                Text(
                    it,
                    style = MaterialTheme.typography.bodySmall,
                    color = MaterialTheme.colorScheme.onSurfaceVariant,
