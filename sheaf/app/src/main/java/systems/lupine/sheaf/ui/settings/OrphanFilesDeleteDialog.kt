...
fun OrphanFilesDeleteDialog(
    ...
    val willQueue = safety.appliesToImages && safety.gracePeriodDays > 0
    ...
    AlertDialog(
        ...
        title = { Text(if (willQueue) "Queue file deletion?" else "Delete unused files?") },
        text = {
            Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
                Text(
                    if (willQueue) {
                        "$fileCount file${plural(fileCount)} ($totalBytesLabel) will be queued " +
                        ...
                        "This will permanently delete $fileCount file${plural(fileCount)} " +
                            "($totalBytesLabel) that are no longer referenced by any member or system. " +
